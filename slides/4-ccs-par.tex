...
\begin{align*}
	...
	\end{align*}}
\only<4>{~\\[-10mm]Mas vamos considerar um operador próprio que permite mais flexibilidade e termos mais concisos. No caso anterior seria:
...
\node[st][ below=1 of S0] (S1){$light.0 |\tau.0$};
\node[st][right of =S1] (S2){$0|\tau.0$};
\node[st][below=1 of S1] (S3){$light.0|0$};
...
\begin{frame}
{Exemplo}
No exemplo anterior calcula  o LTS 	$\bras{(User|Machine)\backslash \{coin,coffeee\}}_\Gamma$ e testa no \href{https://pseuco.com/\#/files}{Pseuco.com} ou no \href{https://lmf.di.uminho.pt/ccs-caos}{ccs-caos}.
...
\begin{frame}{No PseuCo.com / ccs-caos}
...
\end{eqnarray*}
Constrói o LTS a expressão abaixo e testa no \href{https://pseuco.com/\#/files}{Pseuco.com} ou no \href{https://lmf.di.uminho.pt/ccs-caos}{ccs-caos}.
...
\node[st, inner sep=4pt, minimum size=5pt][ below  = of S2] (S3){$(MOF|0|b!.0 )\backslash \{\ell\}$};

\node[st, inner sep=4pt, minimum size=5pt][right  = 0.3cm of S3] (S5){$(MOF|b!.0|0 )\backslash \{\ell\}$};

\node[st, inner sep=4pt, minimum size=5pt][left  = 0.3cm of S3] (S6){$(0|b!.0|b!.0 )\backslash \{\ell\}$};
\node[st][below =of S6] (S7){...};
\node[st][below =of S3] (S8){...};
\node[st][left =of S8] (S10){...};
\node[st][below =of S5] (S9){...};
\node[st][left =of S9] (S11){...};

...
\includegraphics[width=4cm]{src/img/nam/bufbuf}
}
\only<5>{
%\begin{tabular}[{lccr}
\includegraphics[width=3cm]{src/img/nam/bufpass}\\
\includegraphics[width=4cm]{src/img/nam/bufbuf}
%\end{tabular}
}
}
...
\begin{itemize}
	\item $\transtau{s}{\tau}{s'}$ sse $\exists n\geq 0: \transt{s}{\tau^n}{s'}$\\[-7mm]~
	\item $\transtau{s}{a}{s'}$ sse $\exists s'',s'''\in S: \transtau{s}{\tau}{s''}\trans{}{a}{s'''}\transtau{}{\tau}{s'}$
\end{itemize}
Nota que
\begin{itemize}
 \item $\tr\subseteq \trtau$\\[-7mm]~
\item na definição de b.f. podemos usar sempre $\trtau$.\\[-7mm]~
...
% \begin{tabular}{lccr}
\includegraphics[width=4cm]{src/img/nam/bufpass}\\
\includegraphics[width=4cm]{src/img/nam/bufbuf}
% \end{tabular}
}
\end{frame}
...
\item É obtido pelo quociente induzido pela relação de equivalência ($\approx$).
...
% \item Adicionar um lacete com $\tau$ se $P$ tem uma $\tau$-transição para a sua classe $[P]_\simeq$.
% \end{itemize}	
% \end{frame}
...
\item Considerar cada classe de equivalência um estado: $S=\{[s]_{\approx}\mid s\in Reach(P)\}$
\item Adicionar  transições entre duas classes sempre que uma transição exista entre os elementos da respectiva classe.
\item Podemos apagar as transições que seriam adicionadas por $\trtau$ e não estavam em $\tr$.
\item Adicionar um lacete com $\tau$ se $P$ tem uma $\tau$-transição para a sua classe $[P]_\approx$ (apenas para o estado inicial!).
\end{itemize}	
\end{frame}
...
\begin{itemize}
		...
		\item variáveis partilhadas $\alert{b_1}, \alert{b_2}$ e $\alert{k}$, sendo 
		\item se $\alert{k}=i$ então $P_i$ pode entrar
		\item  $P_1$  faz $\alert{k}=2$ (dá o privilégio a $P_2$)
		\item e simétricamente para $P_2$
\item $\alert{b_i}=true$ quando $P_i$ espera
\item  $\alert{b_i}=false$ quando $P_i$ sai da zona crítica.
			\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
Processo $P_i$\\
...
\State{$\alert{b_i}\gets \mathbf{true}$;}
\State{$\alert{k}\gets j$;}
\While{$\alert{b_j} \land \alert{k}=j$} \State{\textbf{skip};}
\EndWhile
	\State {\color{red}critical actions}
\State$\alert{b_i}\gets \mathbf{false};$
		\EndWhile		
\end{algorithmic}
\end{column}
\end{columns}
...
~\\[-12mm]
\begin{eqnarray*}
	\alert{B_{1f}}&:= & b1rf!.\alert{B_{1f}}+b1wf?.\alert{B_{1f}}+b1wt?.\alert{B_{1t}}\\[-1pt]
\alert{B_{1t}}&:= & b1rt!.\alert{B_{1t}}+b1wf?.\alert{B_{1f}}+b1wt?.\alert{B_{1t}}\\[-1pt]
	\alert{B_{2f}}&:= & b2rf!.\alert{B_{2f}}+b2wf?.\alert{B_{2f}}+b2wt?.\alert{B_{2t}}\\[-1pt]
\alert{B_{2t}}&:= & b2rt!.\alert{B_{2t}}+b2wf?.\alert{B_{2f}}+b2wt?.\alert{B_{2t}}\\[-1pt]
	\alert{K_{1}}&:= & kr1!.\alert{K_{1}}+kw1?.\alert{K_{1}}+ kw2?.\alert{K_{2}}\\[-1pt]
\alert{K_{2}}&:= & kr2!.\alert{K_{2}}+kw1?.\alert{K_{1}}+ kw2?.\alert{K_{2}}\\[-1pt]
	P_1&:=& b1wt!.kw2!.P_{11}\\[-1pt]
P_{11}&:=& b2rf?.P_{12} + b2rt?.(kr2?.P_{11} +kr1?.P_{12})\\[-1pt]
P_{12}&:=& enter1.exit1.b1wf!.P_1\\[-1pt]
	P_2&:=& b2wt!.kw1!.P_{21}\\[-1pt]
P_{21}&:=& b1rf?.P_{22} + b1rt?.(kr1?.P_{21} +kr2?.P_{22})\\[-1pt]
P_{22}&:=& enter2.exit2.b2wf!.P_2
\end{eqnarray*}
}{
	Se $k=1$ no início
\begin{eqnarray*}
&&Peterson :=\\&&~~ (P_1|P_2|\alert{B_{1f}}|\alert{B_{2f}}|\alert{K_1})\backslash L	
\end{eqnarray*}
...
Ficheiro no CCS-CAOS:
 \url{https://lmf.di.uminho.pt/ccs-caos/?Peterson}
}

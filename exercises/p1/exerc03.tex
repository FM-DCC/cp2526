...
\begin{eqnarray*}
	CM&:=& coin?.\mathit{coffee}!.CM\\
CS&:=&pub!.coin!.\mathit{coffee}?.CS\\
Uni&:=& (CM|CS)\backslash\{coin,\mathit{coffee}\}
\end{eqnarray*} 
...
\question \pten{Sendo}{Let} $A:=(a.A)\backslash\{b\}$ \pten{mostra que o fragmento atingível}{show that the reachable fragment of} $\bras{A}_\Gamma$ \pten{é infinito}{is infinite}.
\question 
...
\pten{Havia alteração se}{Would it be different if one uses} $User := \overline{p}.enter.\overline{v}.exit.User$ ?
\part \pten{Seja}{Let}
\begin{eqnarray*}
...
\question 
\pten{Modela uma  passagem de nível com 3 componentes: o comboio, a cancela e um controlador (como sugerido na figura). Considera que:}%
{Model a level crossing with three components: the train, the barrier, and a controller. Consider that:}


\begin{itemize}
\item \pten{As acções do comboio  caracterizam a sua localização em relação à passagem: aproxima-se ($approach$), entra ($in$) e sai ($exit$).}{
		The train's actions characterize its location in relation to the crossing:  it approaches
(approach), enters (in), and exits (exit).}
\item \pten{A cancela pode estar baixada  ($down$) ou levantada ($up$).}{
	The barrier can be down or up.}
  \item \pten{O comboio e a cancela não comunicam directamente, apenas o controlador comunica com eles. O controlador deve garantir que quando o  comboio entra a cancela está em baixo.}{
  The train and the barrier do not communicate directly; only the controller communicates with
them. The controller must ensure that when the train enters, the barrier is down.}
  \end{itemize}
  % \begin{center}
...
% \end{center}
\begin{parts}
  \part \pten{Descreve em CCS  cada componente: comboio, cancela e controlador.}{Describe each component in CCS: train, barrier, and controller.}
\part \pten{Escreve o processo do sistema que corresponda à execução paralela das várias componentes, indicando as acções em que devem sincronizar.}{Write the system process that corresponds to the parallel execution of the various components,
indicating the actions in which they must synchronize.}
  \part 
...
\begin{eqnarray*}
C_0&:= &inc. C_1\\	
 C_n&:=& inc.C_{n+1}+dec.C_{n-1}, \text{ \pten{para}{for} } n\geq 1\\
\end{eqnarray*}
e $SC:= inc.(SC|dec.0)$.
\begin{parts}

...
% \begin{center}
\begin{tikzpicture}[>=stealth, shorten >=2.5pt, auto, node distance=2cm,initial text={}]
\node[state, initial, inner sep=1pt, minimum size=15pt] (S0){s};
\node[state,  inner sep=1pt, minimum size=15pt][below  of =S0] (S1){$s_1$};
\node[state,  inner sep=1pt, minimum size=15pt ] [ right  of=S1](S2){$s_2$};
...
\begin{eqnarray*}
	\trans{s_1}{\alpha}{s_2}& \mathit{\pten{sse}{iff}} &	\trans{f(s_1)}{\alpha}{'f(s_2)}
\end{eqnarray*}
\end{itemize}
\begin{parts}
